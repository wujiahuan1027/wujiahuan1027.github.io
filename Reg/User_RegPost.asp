...
<%
Dim EmailCheckReg, AdminCheckReg, EnableMultiRegPerEmail, UserNameLimit, UserNameMax, UserName_RegDisabled, EnableWap, ShowWapShop
Dim PresentExp, PresentMoney, PresentPoint, PresentValidNum, PresentValidUnit, EmailOfRegCheck
Dim EnableCheckCodeOfReg, EnableQAofReg, QAofReg, arrQAofReg, CheckCode
Dim strHTML, PE_Site
...
Sub Main()
    ...
    strFields = "Homepage,主页|QQ,QQ号码|ICQ,ICQ号码|MSN,MSN帐号|Yahoo,雅虎通帐号|UC,UC号码|Aim,Aim帐号|OfficePhone,办公电话|HomePhone,家庭电话|Fax,传真号码|Mobile,手机号码|Region,国家/地区＋省市/州郡＋城市|Address,联系地址|ZipCode,邮政编码|TrueName,真实姓名|Birthday,出生日期|IDCard,身份证号码|Vocation,职业|Company,公司/单位名称|Department,部门名称|PosTitle,职务|Marriage,婚姻状态|Income,收入情况|UserFace,用户头像|FaceWidth,头像宽度|FaceHeight,头像高度|Sign,签名档|Privacy,隐私设定"
    ...
    If UserName = "" Or strLength(UserName) > UserNameMax Or strLength(UserName) < UserNameLimit Then
        FoundErr = True
    ...
    End If
    If UserPassword = "" Or strLength(UserPassword) > 12 Or strLength(UserPassword) < 6 Then
        FoundErr = True
    ...
    If API_Enable Then
        ...
        sPE_Items(conTruename,1) = Trim(Request.Form("TrueName"))
        ...
        sPE_Items(conProvince,1) = Trim(Request.Form("Province"))
        sPE_Items(conCity,1) = Trim(Request.Form("City"))
        sPE_Items(conAddress,1) = Trim(Request.Form("Address"))
        sPE_Items(conZipcode,1) = Trim(Request.Form("ZipCode"))
        sPE_Items(conHomepage,1) = Trim(Request.Form("HomePage"))
    ...
    If EmailCheckReg = True Then
        ...
        Dim PE_Mail
        Set PE_Mail = CreateObject("PE_CMS6.SendMail")
        PE_Mail.iConn = Conn
    ...
    If NeedAddContacter = True Then
        ...
        rsContacter("TrueName") = Trim(Request.Form("TrueName"))
        rsContacter("Title") = Trim(Request.Form("Title"))
        rsContacter("Country") = Trim(Request.Form("Country"))
        rsContacter("Province") = Trim(Request.Form("Province"))
        rsContacter("City") = Trim(Request.Form("City"))
        rsContacter("ZipCode") = Trim(Request.Form("ZipCode"))
        rsContacter("Address") = Trim(Request.Form("Address"))
        rsContacter("Mobile") = Trim(Request.Form("Mobile"))
        rsContacter("OfficePhone") = Trim(Request.Form("OfficePhone"))
        rsContacter("HomePhone") = Trim(Request.Form("HomePhone"))
        rsContacter("PHS") = Trim(Request.Form("PHS"))
        rsContacter("Fax") = Trim(Request.Form("Fax"))
        rsContacter("Homepage") = Trim(Request.Form("Homepage"))
        rsContacter("Email") = Email
        rsContacter("QQ") = Trim(Request.Form("QQ"))
        rsContacter("MSN") = Trim(Request.Form("MSN"))
        rsContacter("ICQ") = Trim(Request.Form("ICQ"))
        rsContacter("Yahoo") = Trim(Request.Form("Yahoo"))
        rsContacter("UC") = Trim(Request.Form("UC"))
        rsContacter("Aim") = Trim(Request.Form("Aim"))
        rsContacter("Company") = Trim(Request.Form("Company"))
        rsContacter("Department") = Trim(Request.Form("Department"))
        rsContacter("Position") = Trim(Request.Form("Position"))
        rsContacter("Operation") = Trim(Request.Form("Operation"))
        rsContacter("CompanyAddress") = Trim(Request.Form("CompanyAddress"))
        rsContacter("BirthDay") = PE_CDate(Trim(Request.Form("BirthDay")))
        rsContacter("IDCard") = Trim(Request.Form("IDCard"))
        rsContacter("NativePlace") = Trim(Request.Form("NativePlace"))
        rsContacter("Nation") = Trim(Request.Form("Nation"))
        ...
        rsContacter("GraduateFrom") = Trim(Request.Form("GraduateFrom"))
        rsContacter("InterestsOfLife") = Trim(Request.Form("InterestsOfLife"))
        rsContacter("InterestsOfCulture") = Trim(Request.Form("InterestsOfCulture"))
        rsContacter("InterestsOfAmusement") = Trim(Request.Form("InterestsOfAmusement"))
        rsContacter("InterestsOfSport") = Trim(Request.Form("InterestsOfSport"))
        rsContacter("InterestsOfOther") = Trim(Request.Form("InterestsOfOther"))
        rsContacter("Family") = Trim(Request.Form("Family"))
        rsContacter("Income") = Trim(Request.Form("Income"))
