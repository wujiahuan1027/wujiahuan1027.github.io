...
If CheckUserLogined() = False Then
    ...
    Response.End
Else
    If UserName = "此处填写用户通用帐号" Then   '通用用户帐号可以公布出来，用于添加文章等操作
        ErrMsg = "<li>通用帐号不可以修改密码</li>"
...
Response.Write "您现在的位置：<a href='../'>" & SiteName & "</a> >> <a href='Index.asp'>会员中心</a> >> 用户管理"
        ...
        <tr>
          <td height="200" valign='top'>
            <%
            Dim PE_UserModule
            Set PE_UserModule = New UserModule
            Call PE_UserModule.Execute
            Set PE_UserModule = Nothing
            %>
          </td>
        </tr>
      </table>
...
'作  用：调用日期 js
'**************************************************
Public Function PopCalendarInit()
    Dim strCalendar
    strCalendar = strCalendar & "<script language='JavaScript' src='PopCalendar.js'></script>" & vbCrLf
    strCalendar = strCalendar & "<script language='JavaScript'>" & vbCrLf
    strCalendar = strCalendar & "PopCalendar = getCalendarInstance()" & vbCrLf
    strCalendar = strCalendar & "PopCalendar.startAt = 0 // 0 - sunday ; 1 - monday" & vbCrLf
    strCalendar = strCalendar & "PopCalendar.showWeekNumber = 0 // 0 - don't show; 1 - show" & vbCrLf
    strCalendar = strCalendar & "PopCalendar.showTime = 0 // 0 - don't show; 1 - show" & vbCrLf
    strCalendar = strCalendar & "PopCalendar.showToday = 0 // 0 - don't show; 1 - show" & vbCrLf
    strCalendar = strCalendar & "PopCalendar.showWeekend = 1 // 0 - don't show; 1 - show" & vbCrLf
    strCalendar = strCalendar & "PopCalendar.showHolidays = 1 // 0 - don't show; 1 - show" & vbCrLf
    strCalendar = strCalendar & "PopCalendar.showSpecialDay = 1 // 0 - don't show, 1 - show" & vbCrLf
    strCalendar = strCalendar & "PopCalendar.selectWeekend = 0 // 0 - don't Select; 1 - Select" & vbCrLf
    strCalendar = strCalendar & "PopCalendar.selectHoliday = 0 // 0 - don't Select; 1 - Select" & vbCrLf
    strCalendar = strCalendar & "PopCalendar.addCarnival = 0 // 0 - don't Add; 1- Add to Holiday" & vbCrLf
    strCalendar = strCalendar & "PopCalendar.addGoodFriday = 0 // 0 - don't Add; 1- Add to Holiday" & vbCrLf
    strCalendar = strCalendar & "PopCalendar.language = 0 // 0 - Chinese; 1 - English" & vbCrLf
    strCalendar = strCalendar & "PopCalendar.defaultFormat = 'yyyy-mm-dd' //Default Format dd-mm-yyyy" & vbCrLf
    strCalendar = strCalendar & "PopCalendar.fixedX = -1 // x position (-1 if to appear below control)" & vbCrLf
    strCalendar = strCalendar & "PopCalendar.fixedY = -1 // y position (-1 if to appear below control)" & vbCrLf
    strCalendar = strCalendar & "PopCalendar.fade = .5 // 0 - don't fade; .1 to 1 - fade (Only IE) " & vbCrLf
    strCalendar = strCalendar & "PopCalendar.shadow = 1 // 0  - don't shadow, 1 - shadow" & vbCrLf
    strCalendar = strCalendar & "PopCalendar.move = 1 // 0  - don't move, 1 - move (Only IE)" & vbCrLf
    strCalendar = strCalendar & "PopCalendar.saveMovePos = 1  // 0  - don't save, 1 - save" & vbCrLf
    strCalendar = strCalendar & "PopCalendar.centuryLimit = 40 // 1940 - 2039" & vbCrLf
    strCalendar = strCalendar & "PopCalendar.initCalendar()" & vbCrLf
    strCalendar = strCalendar & "</script>" & vbCrLf
    PopCalendarInit = strCalendar
End Function

Sub ModifyInfo()
    Dim rsUser,tempUserName
    Set rsUser = Conn.Execute("select * from PE_User where UserID=" & UserID & "")
    tempUserName = rsUser("UserName")
    Response.Write PopCalendarInit() & vbCrLf
    ...
    Response.Write "        <td width='38%'>" & GetGroupName(rsUser("GroupID")) & "</td>" & vbCrLf
    ...
    Response.Write "        <td width='38%'>" & rsUser("UserName") & "</td>" & vbCrLf
    Response.Write "        <td width='12%' class='tdbg5' align='right'>用户密码：</td>" & vbCrLf
    Response.Write "        <td width='38%'><input name='UserPassword' type='text' id='UserPassword' size='20' maxlength='20'> <font color='#FF0000'>不修改请留空</font></td>" & vbCrLf
    Response.Write "    </tr>" & vbCrLf
    Response.Write "    <tr class='tdbg'>" & vbCrLf
    ...
    Response.Write "        <td width='38%'><textarea name='Sign' cols='35' rows='5' id='Sign'>" & PE_ConvertBR(PE_HTMLEncode(rsUser("Sign"))) & "</textarea></td>" & vbCrLf
    ...
    arrEducation = Split(GetArrFromDictionary("PE_Contacter", "Education"), "$$$")
    arrIncome = Split(GetArrFromDictionary("PE_Contacter", "Income"), "$$$")
    ...
    Response.Write "                        <td><input name='Sex' type='radio' value='-1' "
    If Sex < 0 Or Sex > 1 Then Response.Write " checked"
    Response.Write ">保密 <input name='Sex' type='radio' value='1'"
    If Sex = 1 Then Response.Write " checked"
    Response.Write ">男 <input name='Sex' type='radio' value='0'"
    If Sex = 0 Then Response.Write " checked"
    ...
    arrStatusInField = Split(GetArrFromDictionary("PE_Company", "StatusInField"), "$$$")
    arrCompanySize = Split(GetArrFromDictionary("PE_Company", "CompanySize"), "$$$")
    arrManagementForms = Split(GetArrFromDictionary("PE_Company", "ManagementForms"), "$$$")
    If rsUser("UserType") = 1 Then
        Dim rsCompany
    ...
    Response.Write "    <input type='hidden' name='UserName' value='" & tempUserName & "'>" & vbNewLine
...
Sub SaveInfo()
    ...
    If API_Enable Then
        If createXmlDom Then
            ...
            sPE_Items(conQQ,1) = Trim(Request.Form("QQ"))
            sPE_Items(conMsn,1) = Trim(Request.Form("MSN"))
            sPE_Items(conMobile,1) = Trim(Request.Form("Mobile"))
            sPE_Items(conTelephone,1) = Trim(Request.Form("OfficePhone"))
            sPE_Items(conProvince,1) = Trim(Request.Form("Province1"))
            sPE_Items(conCity,1) = Trim(Request.Form("City1"))
            sPE_Items(conAddress,1) = Trim(Request.Form("Address1"))
            sPE_Items(conZipcode,1) = Trim(Request.Form("ZipCode1"))
...
Sub SavePwd()
    Dim OldPassword, Password, PwdConfirm,UserName
    Dim rsUser, sqlUser
...
Public Function PE_ConvertBR(ByVal fString)
    If IsNull(fString) Or Trim(fString) = "" Then
        PE_ConvertBR = ""
        Exit Function
    End If
